...
)
{
	...
	if(pu8Firmware )
	{
		while(1)
		{
			...
			{	
				...
				printf("iReadByteCnt = %d, iToReadBytes=%d\n",iReadByteCnt, iToReadBytes);
			}
			if(iReadByteCnt <= 0)
			{
				printf("Receiving data is incomplete!\n");
			...
			else
			{
				if(iContentLength - iFileBytes - 2*iBoundaryBytes < 4096)
				{
					if(pu8Firmware[iReadByteCnt - 1] == '\n')
					{
						...
						if(strstr(pu8Firmware, strBoundary) != NULL)
						{
							printf("Found boundary at iFileBytes = %d!!!!\n", iFileBytes);
							break;
						}
						else
						{
	...
	if(WEXITSTATUS(iRetCode) != 0)
	{
		...
		//	remove(strFirmwarePath);
			...
			printf("app start update firmware\n");
...
int read_wifistatus(char *strConfigFilePath,cfd lcfd)
{
	...
	if(fConfig == NULL)
	{
		ERROR("Failed to open wifilist file to read!\n");
		return 0;
		//		POST_STATE(buffer,-2);
	...
	if(strConfigFilePath == AP_WIFISTATUS)
	{
		sprintf(strWifiMessage, 
				"{\"ap_ssid\":%s,\"ap_bssid\":\"%s\",\"ap_channel\":\"%s\",\"ap_encryption\":\"%s\",\"ap_addr\":\"%s\",\"ap_mask\":\"%s\",\"ap_gateway\":\"%s\"}", 
				strType[0], strType[1], strType[2], strType[3], strType[4],strType[6],strType[5]);		
		write(lcfd.m_iConnFd, strWifiMessage, strlen(strWifiMessage));
	}else if(strConfigFilePath == STA_WIFISTATUS){
		sprintf(strWifiMessage, 
				",{\"sta_ssid\":%s,\"sta_bssid\":\"%s\",\"sta_channel\":\"%s\",\"sta_encryption\":\"%s\",\"sta_addr\":\"%s\",\"sta_mask\":\"%s\",\"sta_gateway\":\"%s\"}", 
				strType[0], strType[1], strType[2], strType[3], strType[4],strType[6],strType[5]);		
		write(lcfd.m_iConnFd, strWifiMessage, strlen(strWifiMessage));
	...
	while((iReadBytes = ReadLine(pReadBuf, sizeof(pReadBuf), fConfig)) > 0)
	{
		...
		if(j > 5){
			if(!strcmp(strType[1],"unknown")){
				...
				sprintf(strWifiMessage, 
						"{\"ssid\":%s,\"bssid\":\"%s\",\"channel\":\"%s\",\"signal\":\"%s\",\"encryption\":\"%s\"}", 
						strType[1], strType[0], strType[2], strType[3], strType[4]);		
				write(lcfd.m_iConnFd, strWifiMessage, strlen(strWifiMessage));			
...
int DoWifiConfigUpdate(void *arg, S_REQUEST req)
{
	...
	load_config(UCI_WIRELESS_CONFIG);
	while(pToken != NULL)
	{
		...
		if(!strcmp(pParamName[0],"ap_ssid"))
		{
			...
			i_Ret = change_value(UCI_WIRELESS_CONFIG,pParamName[0],pParamName[1],req.m_strParameter);

		}else if(!strcmp(pParamName[0],"ap_auth_key"))
		{
			i_Ret = change_value(UCI_WIRELESS_CONFIG,"ap","key",req.m_strParameter);

		}else if(!strcmp(pParamName[0],"ap_encrypt_type")){

			i_Ret = change_value(UCI_WIRELESS_CONFIG,"ap","encryption",req.m_strParameter);

		}else if(!strcmp(pParamName[0],"ap_channel")){

			i_Ret = change_value(UCI_WIRELESS_CONFIG,"radio0","channel",req.m_strParameter);

		}else if(!strcmp(pParamName[0],"sta_ssid")){

			pParamName[0] = strtok(pParamName[0],"_");
			pParamName[1] = pParamName[0] + strlen(pParamName[0]) + 1;
			i_Ret = change_value(UCI_WIRELESS_CONFIG,pParamName[0],pParamName[1],req.m_strParameter);//pParamValue[0]);

		}else if(!strcmp(pParamName[0],"sta_auth_key")){

			i_Ret = change_value(UCI_WIRELESS_CONFIG,"sta","key",req.m_strParameter);

		}else if(!strcmp(pParamName[0],"sta_encrypt_type")){

			i_Ret = change_value(UCI_WIRELESS_CONFIG,"sta","encryption",req.m_strParameter);

		}else{
			i_Ret = -1;//commands error	
			printf("Not has this option\n");
			return i_Ret;
		}
	...
	if((value_ssid != NULL))
	{
		...
		if(value_key != NULL)
			i_Ret = change_value(UCI_WIRELESS_CONFIG,"sta","encryption",strType[3]);
//		i_Ret = change_value(UCI_WIRELESS_CONFIG,"radio0","channel",strType[1]);

//		i_Ret = change_value(UCI_WIRELESS_CONFIG,"sta","mode","sta");
	}
	unload_config();
...
/*********gz add for upgrade*****************/	
		...
		RK_SndIOCtrl(g_sServer.msg_id, NULL, 0, eIOTYPE_USER_MSG_GPIO_WLED, eIOTYPE_MSG_GPIO_WLED_UPDATE_FLASHING);
    } 
...
/*****************************************add net_api by swt***********************************************/
		...
		if(i_Ret < 0)
		{
			...
			ERROR("could not properly unescape command parameter string\n");
			close(lcfd.m_iConnFd);
			return NULL;
		}
	...
	}else if(pHeadPtr = strstr(buffer,"/param.cgi?action=list&group=wifi")){
		
		req.m_eDataType = A_WIFI_CONFIG;
        ...
        DBG("strParameter (len: %d): \"%s\"\n", len, req.m_strParameter);
        printf("strParameter (len: %d): \"%s\"\n", len, req.m_strParameter);
    }
...
/*********gz add for upgrade end************/				
    ...
    case A_STREAM:
       ...
       // send_stream(lcfd.m_iConnFd, input_number);
        break;
	case A_DEV_NAME:
		DBG("Request for device name.services.\n");
	...
	case A_UPGRADE:
		{
			...
			{	
				printf("Receiving data message is error !\n");
		...
		//	op_data.date = &u8date;
			...
			if (ret <0)
			{
				DBG(" send bt cmd error !!!\n");
			}
			printf("iOtaContentLengths: %d\n", iOtaContentLengths);	//file length;
	...
	case A_REST_BOARD:
		{
			i_Ret = RK_SndIOCtrl(lcfd.m_psContext->msg_id, NULL, 0, eIOTYPE_USER_MSG_AVS, eIOTYPE_MSG_AVSLOGOUT);
			POST_STATE(buffer,i_Ret);
			write(lcfd.m_iConnFd,buffer,strlen(buffer));
			i_Ret = RK_SndIOCtrl(lcfd.m_psContext->msg_id, NULL, 0, eIOTYPE_USER_MSG_GPIO_WLED, eIOTYPE_MSG_GPIO_WLED_OFF);
	...
	case A_RESTART_NET:
		{
			...
			const char *value_ssid = search_config("sta","ssid");
			if(value_ssid != NULL)
				i_Ret = change_value(UCI_WIRELESS_CONFIG,"radio0","linkit_mode","sta");
			else
				mled_flag = 1;
			unload_config();
			pthread_mutex_unlock(&g_hMutex);
	...
	case A_WIFI_CONFIG:
		{
			...
			if((value = search_config("radio0","channel")) != NULL)
				strcpy(strType[0],value);
			else
				strcpy(strType[0],"none");

			if((value = search_config("radio0","linkit_mode")) != NULL)
				strcpy(strType[11],value);
			else
				strcpy(strType[11],"none");
			
			if((value = search_config("ap","network")) != NULL)
				strcpy(strType[1],value);
			else
				strcpy(strType[1],"none");

			if((value = search_config("ap","mode")) != NULL)
				strcpy(strType[2],value);
			else
				strcpy(strType[2],"none");

			if((value = search_config("ap","ssid")) != NULL)
				strcpy(strType[3],value);
			else
				strcpy(strType[3],"none");

			if((value = search_config("ap","encryption")) != NULL)
				strcpy(strType[4],value);
			else
				strcpy(strType[4],"none");

			if((value = search_config("ap","key")) != NULL)
				strcpy(strType[5],value);
			else
				strcpy(strType[5],"none");

			if((value = search_config("sta","network")) != NULL)
				strcpy(strType[6],value);
			else
				strcpy(strType[6],"none");

			if((value = search_config("sta","mode")) != NULL)
				strcpy(strType[7],value);
			else
				strcpy(strType[7],"none");

			if((value = search_config("sta","ssid")) != NULL)
				strcpy(strType[8],value);
			else
				strcpy(strType[8],"none");

			if((value = search_config("sta","encryption")) != NULL)
				strcpy(strType[9],value);
			else
				strcpy(strType[9],"none");

			if((value = search_config("sta","key")) != NULL)
				strcpy(strType[10],value);
			else
				strcpy(strType[10],"none");
			...
			write(lcfd.m_iConnFd,DEF_JSON_HEADER,strlen(DEF_JSON_HEADER));
					sprintf(strWifiMessage, 
							"{\"wifi_config\":[{\"link_mode\":\"%s\",\"ap_channel\":\"%s\",\"ap_network\":\"%s\",\"ap_mode\":\"%s\",\"ap_ssid\":\"%s\",\"ap_encryption\":\"%s\",\"ap_key\":\"%s\"},{\"sta_network\":\"%s\",\"sta_mode\":\"%s\",\"sta_ssid\":\"%s\",\"sta_key\":\"%s\",\"sta_encryption\":\"%s\"}]}", 
							strType[11],strType[0], strType[1], strType[2], strType[3], strType[4],strType[5],strType[6],strType[7],strType[8],strType[9],strType[10]);		
					write(lcfd.m_iConnFd, strWifiMessage, strlen(strWifiMessage));			
		}
		break;
	...
	case A_LOGIN_GET_DATA:
		{
			...
			RK_SndIOCtrl(lcfd.m_psContext->msg_id, NULL, 0, eIOTYPE_USER_MSG_AVS, eIOTYPE_MSG_AVSLOGIN_GETAUTH);

			//(SMsgIoctrlAvsGetAuth *)msgData->next = msg_avs_get_data;
	...
	case A_LOGIN_SET_DATA:
		{
			i_Ret = RK_SndIOCtrl(lcfd.m_psContext->msg_id, msg_avs_set_data, sizeof(SMsgIoctrlAvsSetAuth), eIOTYPE_USER_MSG_AVS, eIOTYPE_MSG_AVSLOGIN_SETTOKEN);
			SMsgIoctrlData *msgData = (SMsgIoctrlData *)malloc(sizeof(SMsgIoctrlData) + 1);
			if(msgData == NULL)
				i_Ret = -2;

			memset(msgData,0,sizeof(msgData));

			RK_RecvIOCtrl(lcfd.m_psContext->msg_id, msgData, sizeof(SMsgIoctrlData) + 1, eIOTYPE_USER_MSG_HTTPD);
			if(msgData->u32iCommand == eIOTYPE_MSG_AVSLOGIN_RESPONSE)
			{
				i_Ret = msgData->next[0];
			}
			free(msgData);
	...
	case A_OPEN_TUNNEL:
		{
			i_Ret = RK_SndIOCtrl(lcfd.m_psContext->msg_id, NULL, 0, eIOTYPE_USER_MSG_TUNNEL, eIOTYPE_MSG_TUNNEL_START);
			POST_STATE(buffer,i_Ret);
			write(lcfd.m_iConnFd,buffer,strlen(buffer));
		}
		break;
	...
	case A_CLOSE_TUNNEL:
		{
			i_Ret = RK_SndIOCtrl(lcfd.m_psContext->msg_id, NULL, 0, eIOTYPE_USER_MSG_TUNNEL, eIOTYPE_MSG_TUNNEL_STOP);
			POST_STATE(buffer,i_Ret);
			write(lcfd.m_iConnFd,buffer,strlen(buffer));
		}
		break;
	...
	case A_SET_BAUDRATE:
		{
			free_tunnelflag = 1;
			i_Ret = RK_SndIOCtrl(lcfd.m_psContext->msg_id, msg_tunnel_data, sizeof(SMsgIoctrlTunnel), eIOTYPE_USER_MSG_TUNNEL, eIOTYPE_MSG_TUNNEL_SETBAUDRATE);
			POST_STATE(buffer,i_Ret);
			write(lcfd.m_iConnFd,buffer, strlen(buffer));	
	...
	case A_GET_BAUDRATE:
		{
			...
			RK_SndIOCtrl(lcfd.m_psContext->msg_id, NULL, 0, eIOTYPE_USER_MSG_TUNNEL, eIOTYPE_MSG_TUNNEL_GETBAUDRATE);
	...
	case A_SET_TUNNEL_PORT:
		{
			free_tunnelflag = 1;
			i_Ret = RK_SndIOCtrl(lcfd.m_psContext->msg_id, msg_tunnel_data, sizeof(SMsgIoctrlTunnel), eIOTYPE_USER_MSG_TUNNEL, eIOTYPE_MSG_TUNNEL_SETPORT);
			POST_STATE(buffer,i_Ret);
			write(lcfd.m_iConnFd,buffer, strlen(buffer));	
		}
		break;
	...
	case A_GET_TUNNEL_PORT:
		{
			...
			RK_SndIOCtrl(lcfd.m_psContext->msg_id, NULL, 0, eIOTYPE_USER_MSG_TUNNEL, eIOTYPE_MSG_TUNNEL_GETPORT);
...
void *server_thread(void *arg)
{
	...
	while ( 1 ) {
		...
		DBG("create thread to handle client that just established a connection:%s\n", inet_ntoa(client_addr.sin_addr));
		//syslog(LOG_INFO, "serving client: %s:%d\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
		if(g_uiConnThreadCnt >= g_sServer.conf.m_u16ConnLimitCnt)
		{
			ERROR("Out of connection limitation!\n");
			...
			if(g_sServer.conf.m_u16Timeout > 0)
			{
				...
				if(setsockopt(pcfd->m_iConnFd, SOL_SOCKET, SO_SNDTIMEO, &sTimeout, sizeof(sTimeout)) < 0)
					ERROR("Failed to setsockopt SO_SNDTIMEO!\n");
			}
