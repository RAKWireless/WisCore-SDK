...
int RK_GpioOpen(int port , int dir)
{
	...
	if ((fp = fopen("/sys/class/gpio/export", "w")) == NULL) {
		RK_ERROR("Cannot open export file.\n");
		return(-1);
	}
	fprintf(fp, "%d", port); 
	fclose(fp);
	...
	if ((fp = fopen(port_str, "rb+")) == NULL) {
		RK_ERROR("Cannot open direction file\n [%s]\n" , port_str);
		return(-1);
	}
...
int RK_GpioWrite(int port, int value)
{
	...
	sprintf(port_str , "/sys/class/gpio/gpio%d/value" , port);
	if ((fp = fopen(port_str, "rb+")) == NULL) {
		RK_ERROR("Cannot open value file\n [%s]\n", port_str);
		return(-1);
	}
	if(value)
		fwrite("1", 1, 1, fp);
...
int RK_GpioRead(int port)
{
	...
	sprintf(port_str , "/sys/class/gpio/gpio%d/value" , port);
	if ((fp = fopen(port_str, "rb+")) == NULL) {
		RK_ERROR("Cannot open value file for input\n[%s]\n",port_str);
		return(-1);
	}
...

{
	...
	if (fd < 0) {  
		RK_ERROR("Failed to open gpio edge for writing!\n");  
		return -1;  
	}  

	if ((write(fd, &dir_str[ptr], strlen(&dir_str[ptr]))) < 0) {  

		RK_ERROR("Failed to set edge!\n");  
